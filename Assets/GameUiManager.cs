...
public class GameUiManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        freezeButton.onClick.AddListener(OnClickFreezeButton);
    }

    private void OnClickFreezeButton()
    {
        if (GameManager.instance.freezeAmount > 0)
        {
            freezeImage.gameObject.SetActive(true);
            ManagerGame.TIME_SCALE = 0;
            StartCoroutine(ChangeState());
            GameManager.instance.freezeAmount --;
            freezeText.text = GameManager.instance.freezeAmount.ToString();
            PlayerPrefs.SetInt(PlayerPrefsManager.freeze, GameManager.instance.freezeAmount);
        }
